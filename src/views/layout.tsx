...
export function Layout({
  ...
  const title = meta.title
    ...
    : config.appName;
  const description =
    meta.description ?? "Latihan mengetik dengan leaderboard real-time.";
  const navLinks = [
    { href: "/", label: "Beranda" },
  ...
  return (
    <html lang="id">
      <head>
        ...
        {/* app.js harus terdaftar sebelum alpine.min.js (defer dijalankan berurutan)
            agar listener `alpine:init` sudah siap sebelum Alpine auto-start */}
      ...
      <body class={`${bodyClass}`}>
        <header class="sticky top-0 z-40 border-b border-line bg-surface/80 backdrop-blur">
          <div class="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between gap-4">
            ...
            >
              <span class="w-8 h-8 rounded-lg bg-accent text-white grid place-items-center font-mono text-sm">
                ⌨
              </span>
              {config.appName}
            </a>
            <nav class="hidden md:flex items-center gap-1 text-sm">
              {navLinks.map((l) => (
                <a
                  href={l.href}
                  class="px-3 py-2 rounded-lg text-ink-soft hover:text-ink hover:bg-base transition-colors"
                >
                  {l.label}
                </a>
              ))}
            ...
            <div class="flex items-center gap-2 text-sm">
              {user ? (
                <>
                  <a
                    href="/sesi"
                    class="hidden sm:inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-base text-ink-soft"
                  >
                  ...
                  <a
                    href="/logout"
                    class="px-3 py-1.5 rounded-lg border border-line hover:border-ink transition-colors"
                  >
                    Keluar
                  </a>
                </>
                ...
                <>
                  ...
                  <a
                    href="/register"
                    class="px-3 py-1.5 rounded-lg bg-ink text-white hover:bg-ink/90 transition-colors"
                  >
                    Daftar
                  </a>
                </>
